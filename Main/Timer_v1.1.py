...
from pygame import mixer # This program uses the PyGame Mixer to utilize sounds
...
def timer(timer_Length, list_Iteration): # The actual timer portion of the program
    ...
    try:
        for i in range(timer_Length*60): # timer_length*60 Converts minutes to Seconds
            ...
            # The sleep function suspends the execution of all code for 1 second 
            time.sleep(1) #(for debugging, set this value to 0.1)
        cls()
        return 1
        ...
        if list_Iteration == 0: # Embarasses user for skipping work session 
            mixer.Sound(filePath + "\haaaNikkiMinaj.wav").play()
            print("pfffffftt")
            time.sleep(2) # Makes sure Nikkis laugh plays all the way through before being interrupted by Travis Scott
        return 0
